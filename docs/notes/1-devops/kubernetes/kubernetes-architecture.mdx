...
You rarely need to install these components manually. For development, tools like **minikube** create a single-node cluster on your local machine. For production, managed services like **Amazon EKS** or **Google GKE** provide pre-configured clusters. For on-premises setups, **kubeadm** is a popular installation utility.

For a deeper educational experience, Kelsey Hightower's **Kubernetes the Hard Way** tutorial guides you through a manual installation on bare Linux machines. While challenging, it provides invaluable insight into the inner workings of Kubernetes.
...
Add-ons use Kubernetes resources like Deployments and DaemonSets to implement cluster-level features. They typically run in the `kube-system` namespace.
...
It is a stateless Go application, and its source code is available on [GitHub](https://github.com/kubernetes/kubernetes) under the Apache 2.0 license. To instruct Kubernetes, you send an HTTP request to a `kube-apiserver` endpoint. It is the single entry point for all create, update, and delete operations.
...
Notice there is no "container" resource. Kubernetes abstracts containers using a resource called a **Pod**. A Pod can hold one or more containers, though a single-container Pod is the most common use case. For more advanced patterns, see our guide on [multi-container Pods and design patterns](./multi-container-pods-and-design-patterns).
...
Some lightweight Kubernetes distributions like K3s can use alternatives like SQLite or even external databases like MySQL, but `etcd` is the standard for most clusters.
...
Now let's examine the components that run on every compute (worker) node.
...
While Docker was the original default for Kubernetes, the ecosystem has shifted towards runtimes that adhere to the **Container Runtime Interface (CRI)**. CRI is a plugin interface that allows the `kubelet` to communicate with various container runtimes without needing custom integrations.
...
The `kubelet` acts as a bridge between the `kube-apiserver` and the local container runtime. It periodically polls the API server (by default, every 20 seconds) to get the list of Pods that have been scheduled to its node.
...
While you could technically use `curl` to interact with the API, `kubectl` simplifies the process immensely. It handles authentication, manages cluster contexts, and provides easy-to-remember commands for complex operations. Mastering `kubectl` is essential for being effective with Kubernetes.
...
YAML ("YAML Ain't Markup Language") is a human-readable data serialization format. It is not a programming language but a way to define configuration using key-value pairs.
...
The declarative approach enables **Infrastructure as Code (IaC)**. You can store your YAML files in a Git repository, track changes over time, and easily recreate your applications. You can also define multiple resources in a single file, separated by `---`.
...
Mastering both imperative and declarative methods is key, as each has its strengths and is suited for different tasks.
