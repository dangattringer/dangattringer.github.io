...
It's important to understand that Kubernetes doesn't replace container runtimes like [Docker](../Docker/index.md). Instead, it works *with* them. Kubernetes manages the **clusters** of machines (nodes) where these container runtimes are running, orchestrating the containers across the available infrastructure.
